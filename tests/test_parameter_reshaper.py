...
def test_parameter_reshaper(batch_shape, moveaxis):
    reshaper = sus.ParameterReshaper({
        'a': None,
        'b': (4,),
        'd': 9,
        'c': (9, 7),
...
def test_invalid_array_size(shape):
    reshaper = sus.ParameterReshaper({'a': 3})
    x = np.random.normal(0, 1, shape)
...
def test_inconsistent_batch_shape():
    reshaper = sus.ParameterReshaper({'a': 3, 'b': 2})
    y = {
...
def test_extraneous_key():
    reshaper = sus.ParameterReshaper({'a': 2})
